  ...
  return (
    <div className="departure-container">
      {departureList?.stops?.length && departureList?.departures?.length ? (
        <div>
          <div className="departure">
            ...
            <div className="stop-description">
              <div className="stop-name">
                <h3>{departureList?.stops[0]?.description}</h3>
              </div>
              <div className="stop-number">
                Stop {departureList?.stops[0]?.stop_id}
              </div>
            </div>
          </div>
          <div role="list">
            {departureList?.departures
              ?.slice(0, departureCountToShow)
              .map((departure, index) => {
                return (
          ...
          {departureList.departures.length > 3 && (
            <button className="show-more-button" onClick={showMoreDeparture}>
              <span
                className={departureExpanded ? "expand" : "colapsed"}
              ></span>
        ...
        <div>
          unable to get the direction for the above selection, try chaning the
          option
        </div>
      )}
    </div>
